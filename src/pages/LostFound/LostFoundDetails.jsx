            ...
            >
              ...
              >
                ...
                <div style={{ marginTop: 12, display: "grid", gap: 10 }}>
                  <div><strong>Breed:</strong> {item?.breed || "Unknown"}</div>
                  <div><strong>Color:</strong> {item?.color || "Unknown"}</div>
                  <div><strong>Gender:</strong> {item?.gender || "Unknown"}</div>
                  <div><strong>City:</strong> {item?.location?.city || "—"}</div>
                  <div><strong>ZIP:</strong> {item?.location?.zip || "—"}</div>
                ...
                <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginTop: 16 }}>
                  ...
                  <button
                    type="button"
                    className="pill"
                    onClick={() => navigate(`/lost-found/${encodeURIComponent(uid || "")}/sighting`)}
                  >
                    I Have Seen This Pet
                  </button>
                  <button type="button" className="pill" onClick={() => toast.info("We’ll add “Reunited” workflow next.")}>
                    Mark as Reunited
                  </button>
                </div>
              </div>
